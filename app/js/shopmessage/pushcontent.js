...
$(function(){
	...
	mui.plusReady(function(){
	    ...
	    window.currentname = shopinfo.user_info.name;
...
//	点击跳转到编辑
	$('html').on('click','.editbtn',function(){
		...
		$('.titlestype').css('border','1px solid #000');
		$('.contenttype').removeAttr('disabled');
		$('.contenttype').css('border','1px solid #000');
		$(this).hide();
		$('.sendbtn').show();
...
//		})
	...
	$('html').on('click','.sendbtn',function(){
		...
		var url = rooturl+'index.php/Api/Apps/Jingkaiqu/Notify/editNotify'; //编辑推送
		$.ajax({
			url: url,
			data: {
				mobile: mobile,
