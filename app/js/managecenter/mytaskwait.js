...
//					document.getElementsByClassName('mui-table-view-item')[0].firstChild().style.display = 'block';
//					mui('.mui-table-view-item').eq(a).find('.left-checkbox').show();
				}
...
//	返回刷新
	...
	function pulldownRefresh() {
		...
		mui('#waitrefresh').pullRefresh().refresh(true);
		setTimeout(function() {
			firstgetmytask('wait','','');
			refreshcancel();
			getformtype();
		...
		$.ajax({
			...
			//		// 抓取模板数据
					...
					if(window.pulldown == 'true'){
						...
						mui('#waitrefresh').pullRefresh().endPullupToRefresh(false);
						mui('#waitrefresh').pullRefresh().endPulldownToRefresh();
						setTimeout(function() {
							var prev = plus.webview.currentWebview().opener();
