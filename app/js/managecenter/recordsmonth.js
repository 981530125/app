...
//			getshop('',searchkey,page,area_id,true,'','');
	...
	$('html').on('click','.search-btn',function(){
		...
		$("#my-mask").show();
		var searchkey = $('.searchkey').val();
		firstgetmyrecords('month','',searchkey);
	})
	...
	function getmytask(recordType,page,searchkey,selfOnly){
		...
		$.ajax({
			...
			//		// 抓取模板数据
					...
					mui('#monthrefresh').pullRefresh().endPullupToRefresh(false);
				}else{
					mui('#monthrefresh').pullRefresh().endPullupToRefresh(true);
		...
		$.ajax({
			...
			success: function(res) {
				if(res.code && res.code == 1000){
					...
					mui('#monthrefresh').pullRefresh().endPulldown();
				}
				$("#my-mask").hide();
			},
			error: function(res){
