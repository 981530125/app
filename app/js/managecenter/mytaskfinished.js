$(function(){
	...
	function pulldownRefresh() {
			pulldown = 'false';
			mui('#finishedrefresh').pullRefresh().refresh(true);
			firstgetmytask('finished',1,'');
			refreshcancel();
			getformtype();
	}
	var waitcount = 1;
...
//	删除方法
	...
	$('html').on('click','.search-btn',function(){
		...
		$("#my-mask").show();
		var searchkey = $('.searchkey').val();
		firstgetmytask('finished','',searchkey);
	})
...
//		mui('#processrefresh').pullRefresh().endPullupToRefresh((count > allnum));
		...
		$.ajax({
			...
			//		// 抓取模板数据
					...
					mui('#finishedrefresh').pullRefresh().endPullupToRefresh(true);
	...
	function firstgetmytask(taskType,page,searchkey){
		...
		$.ajax({
			...
			//		// 抓取模板数据
				...
				$("#my-mask").hide();
				if(pulldown == 'false') {
					mui('#finishedrefresh').pullRefresh().endPulldownToRefresh();
				}
			},
			error: function(res){
				console.log(JSON.stringify(res));
