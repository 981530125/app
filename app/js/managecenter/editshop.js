...
$(function(){
	...
	var rootpower = [{
			obj:'.editshopinfo',
			apiurl:'/index.php/Api/Apps/Jingkaiqu/Shop/editLicenseShopInfo',
			describe:'修改企业信息'
		},{
			obj:'.editmycerbook',
			apiurl:'/index.php/Api/Apps/Jingkaiqu/License/editLicenseInfo',
			describe:'编辑证书'
		},{
			obj:'.healthcertificate',
			apiurl:'/index.php/Api/Apps/Jingkaiqu/License_health_certificate_file/getFile',
			describe:'获取健康证'
		},{
			obj:'#material',
			apiurl:'/index.php/Api/Apps/Jingkaiqu/Investigate_record_file/getInvestigateRecordFile',
			describe:'获取辅助材料'
		}];
	//获取跳转参数
...
//	年度量化综合等级
	...
	function changeicon(node,val,non){
		$('html').on('click',node,function(){
			...
			$(non).css('background-color','#ebebeb');
			switch(level)
			{
			case 'A':
			...
			case 'D':
				...
				console.log(level);
			  	break;
			default:
				$(this).removeClass('nowait-think');
				$(this).css('background-color','#32a8dd');
//				$(this).addClass('wait-think');
			  	console.log(level);
			}
	...
	function editshopinfo(){
		...
		$.ajax({
			...
			data: {
				...
				three_small_status:formjson.three_small_status,
				loginway:loginway
			},
			dataType: 'json',
			success: function(res) {
 		...
 		$('html').on('click','.mui-switch',function(event){
 			...
 			if($(this).hasClass('mui-active')){
 				for(var n = 0;n<num;n++){
 					...
 					$('.mui-switch').eq(n).find(".hide-switch").html('不公开');
 				}
 			}else{
 				for(var n = 0;n<num;n++){
 					$('.mui-switch').eq(n).addClass('mui-active');
 					$('.mui-switch').eq(n).find(".is_open").val('1');
 					$('.mui-switch').eq(n).find(".hide-switch").html('公开');
 				}
 			}
 		})
 	})
