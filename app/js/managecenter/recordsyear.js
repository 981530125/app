...
//	返回刷新
	...
	function pulldownRefresh() {
		setTimeout(function() {
			reload();
//			mui('#waitrefresh').pullRefresh().endPulldownToRefresh(); //refresh completed
		}, 1000);
	}
...
//			getshop('',searchkey,page,area_id,true,'','');
	...
	$('html').on('click','.search-btn',function(){
		...
		$("#my-mask").show();
		var searchkey = $('.searchkey').val();
		firstgetmyrecords('year','',searchkey);
	})
	...
	function getmytask(recordType,page,searchkey,selfOnly){
		...
		$.ajax({
			...
			//		// 抓取模板数据
					...
					mui('#yearrefresh').pullRefresh().endPullupToRefresh(false);
				}else{
					mui('#yearrefresh').pullRefresh().endPullupToRefresh(true);
		...
		$.ajax({
			...
			success: function(res) {
				if(res.code && res.code == 1000){
					...
					mui('#yearrefresh').pullRefresh().endPulldownToRefresh();
				}else{
					if(tasklist == 'null'){
						var contenttpl = $("#none-template").html();
					}else{
					...
					mui('#yearrefresh').pullRefresh().endPulldown();
					return false;
				}
				$("#my-mask").hide();
			},
