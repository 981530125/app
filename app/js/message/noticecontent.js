...
//	编辑,回复,转发未做权限判断
	...
	var rootpower = [{
			...
			obj:'.replysubmit',
			apiurl:'/index.php/Api/Apps/Jingkaiqu/Message/editMessageData'
		},{
			obj:'#add-message',
			apiurl:'/index.php/Api/Apps/Jingkaiqu/Message/getMessageDetailByMessageId'
		}];
...
//	    var admininfo = JSON.parse(localStorage.getItem("admininfo"));
	...
	function getmessagecontent(id){
		var url = rooturl+'index.php/Api/Apps/Jingkaiqu/Message/getMessageDetailByMessageId'; //获取信息详情
		$.ajax({
			url: url,
			data: {
				mobile: mobile,
...
//	点击返回
	...
	function submitdeal(title,content,received_admin_id,root_message_id,status,file_id_list){
		var url = rooturl+'index.php/Api/Apps/Jingkaiqu/Message/editMessageData'; //获取处置
		$.ajax({
			url: url,
			data: {
				mobile: mobile,
