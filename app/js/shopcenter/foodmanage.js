...
//	获取自身菜品
	function getactivity(searchkey,review_status){
		...
		var url = rooturl+'index.php/Api/Apps/Jingkaiqu/Shop_goods/getSelfShopGoods';
		$.ajax({
			url: url,
			data: {
				mobile: mobile,
			...
			//		// 抓取模板数据
					...
					// 抓取模板数据
					var nonecontenttpl = $("#nodata-template").html();
					// 编译模板
					var nonedatacontenttp = Handlebars.compile(nonecontenttpl);
					var data = res;
					// 把数据传送到模板
					var text = nonedatacontenttp(data);
					// 更新到模板
					$('#datalist').html(text);
