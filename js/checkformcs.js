...
$(function() {
	...
	mui.plusReady(function() {
		...
		console.log(JSON.stringify(window.type_of));
...
//			调整设置默认数据
//			if(tablename == 'checklist__notification_form' || tablename == 'checklist__notification_form' ){}
...
//			生成需要获取表单的数组
			var existarr = [];
			var existarrname = [];
			for(var imt in existjson){
				existarr.push(existjson[imt].en_name);
				existarrname.push(existjson[imt].cn_name);
			}
			
			for(var c = 0;c<existarr.length;c++){
				var target = existarr[c];
				if($.inArray(target,currentchecklist) == '-1'){
					console.log('不存在'+target);
					console.log('不存在'+existarrname[c]);
					
					window.current_enname = target;
					window.current_formname = existarrname[c];
...
//			prompt('asas',JSON.stringify(window.forminfo.checklist));
//			prompt('sss',JSON.stringify(checklist));
...
//					prompt("打印錯誤res", JSON.stringify(res));
		...
		addformitem = function() {
			...
			console.log(JSON.stringify(formnamelist));
